...
def detect_role(query):
    query_lower = query.lower()  # Преобразуем запрос в нижний регистр
    if "role_pm" in query_lower or "project manager" in query_lower:
        return "role_PM"
    elif "role_employee" in query_lower or "Что я могу ожидать от своего PO/PM" in query_lower or "junior" in query_lower:
        return "role_employee"
    elif "role_lead" in query_lower:
        return "role_lead"
    else:
        return "other"
...
# Определение роли на основе запроса
"""def detect_role(query):
    if "role_PM" in query.lower() or "project manager" in query.lower():
        return "role_PM"
    elif "role_employee" in query.lower():
        return "role_employee"
    elif "role_lead" in query.lower():
        return "role_lead"
    else:
        return "other"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    # Определяем роль для поиска
    search_role = detect_role(question)
    if search_role in role_to_index:
        print(f"🔍 Используем индекс для роли: {search_role}")  # Логирование
