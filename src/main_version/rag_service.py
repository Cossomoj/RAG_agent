...
1. **Заголовки**: 
   - # Основной заголовок (только один в начале)
   - ## Разделы (например: ## Product Owner (PO):)
   - ### Подразделы (например: ### Middle-аналитик)
   - Всегда ставь пробел после символов #

2. **Списки**: 
   - Для нумерованных списков: 1. 2. 3. (с пробелом после точки)
   - Для маркированных списков: - пункт (с пробелом после тире)
   - Каждый пункт списка начинается с новой строки
   - После двоеточия (:) переходи на новую строку для списка

3. **Выделение текста**:
   - **Жирный текст** для ключевых терминов (двойные звездочки)
   - *Курсив* для акцентов (одинарные звездочки)
   - `код` для технических терминов (обратные кавычки)

4. **Структура ответа**:
   - Используй пустые строки между разделами
   - Группируй связанную информацию под заголовками
   - Избегай длинных параграфов без разбивки

5. **Примеры правильного форматирования**:
   ```
   ## Заголовок раздела
   
   Вводный текст с **важными** терминами.
