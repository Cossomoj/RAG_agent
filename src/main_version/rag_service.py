...
КРИТИЧЕСКИ ВАЖНО - ФОРМАТИРОВАНИЕ И СОДЕРЖАНИЕ ОТВЕТА:
Отформатируй свой ответ строго в формате Markdown.
Твой ответ должен основываться ИСКЛЮЧИТЕЛЬНО на предоставленном контексте.
НЕ добавляй никакой информации, которой нет в тексте. Если в контексте нет прямого ответа, сообщи об этом.

1.  **ЗАГОЛОВКИ** (обязательно с пробелами после #):
    *   `# Основной заголовок ответа`
    *   `## Подзаголовок`
    *   `### Детальный подзаголовок`

2.  **СТРУКТУРА**:
    *   Начинай с основного заголовка `#`.
    *   Используй подзаголовки `##` и `###` для логического разделения.
    *   Обязательно оставляй пустую строку после каждого заголовка.
...
5.  **ЗАПРЕЩЕНО**:
    *   Придумывать факты или детали, отсутствующие в контексте.
    *   Использовать `###` или `##` в середине текста без создания заголовка.
    *   Писать длинные абзацы без разбивки.
    *   Забывать пустые строки между разделами.
