...
create_env_file() {
    ...
    if [ ! -f "$ENV_FILE" ]; then
        log "Файл .env не найден, создаем шаблон без токенов..."
        cat > "$ENV_FILE" << 'EOF'
# Токены для сервисов (необходимо заполнить вручную)
# GigaChat API токен
GIGACHAT_API_KEY=
...
EOF
        ...
        log "Шаблон файла .env создан. ВНИМАНИЕ: необходимо заполнить токены вручную!"
        log "Редактируйте файл: $ENV_FILE"
    else
        log "Файл .env уже существует, проверяем наличие всех необходимых переменных..."
        # Проверяем и добавляем отсутствующие переменные без перезаписи значений
        ensure_env_variable_template "$ENV_FILE" "GIGACHAT_API_KEY"
        ensure_env_variable_template "$ENV_FILE" "TELEGRAM_API_KEY"
        ensure_env_variable_template "$ENV_FILE" "TELEGRAM_BOT_TOKEN"
        ensure_env_variable_template "$ENV_FILE" "TELEGRAM_CHAT_ID"
        ensure_env_variable_template "$ENV_FILE" "FEEDBACK_CHAT_ID"
        ensure_env_variable_template "$ENV_FILE" "FEEDBACK_BOT_TOKEN"
        ensure_env_variable_template "$ENV_FILE" "TELEGRAM_BOT_ACCESS"
        ensure_env_variable_template "$ENV_FILE" "CHAT_ID"
        ensure_env_variable_template "$ENV_FILE" "ALERT_BOT_TOKEN"
    fi
}

# Функция для добавления переменной в .env файл, если она отсутствует (без значения)
ensure_env_variable_template() {
    local env_file=$1
    local var_name=$2
    
    if ! grep -q "^$var_name=" "$env_file"; then
        # Переменная не существует, добавляем шаблон
        echo "$var_name=" >> "$env_file"
        log "Добавлена переменная $var_name в файл .env (требуется заполнить значение)"
    fi
}
