...
# Отдельный объект бота для рассылки (решение проблемы 409)
newsletter_bot = telebot.TeleBot(secret_key)

# Thread-safe очередь для отправки сообщений
...
class MessageStats:
    ...
    def add_api_error(self, error):
        with self.lock:
            self.api_errors.append({
                'error': str(error),
                'timestamp': datetime.now()
            })
            # Сохраняем только последние 10 ошибок
            if len(self.api_errors) > 10:
                self.api_errors.pop(0)
...
def get_message_queue_status():
    """Получить статус очереди сообщений"""
    stats = message_stats.get_stats()
    stats['queue_size'] = message_queue.qsize()
    stats['worker_active'] = message_sender_active
    return stats

def message_sender_worker():
    """
    Отдельный поток для отправки сообщений из очереди
    ...
    logger.info("📨 Message sender worker запущен")
    
    while message_sender_active:
        try:
            # Получаем сообщение из очереди (блокирующий вызов с таймаутом)
            message_data = message_queue.get(timeout=1)
            
            if message_data is None:  # Сигнал для завершения
                logger.info("📨 Получен сигнал завершения message sender worker")
                break
            ...
            # Обновляем статистику размера очереди
            message_stats.queue_size = message_queue.qsize()
            ...
            try:
                ...
                if "Forbidden: bot was blocked by the user" in str(e):
                    ...
                    message_stats.add_blocked_user(chat_id)
                else:
                    logger.error(f"❌ Ошибка Telegram API при отправке сообщения пользователю {chat_id}: {e}")
                    message_stats.add_api_error(e)
                message_stats.increment_failed()
            ...
            logger.error(f"Критическая ошибка в message_sender_worker: {e}")
            message_stats.add_api_error(e)
    
    logger.info("📨 Message sender worker завершен")

def safe_send_message(chat_id, text, reply_markup=None, priority=False):
    """
    Thread-safe функция для отправки сообщений через очередь
    ...
    Args:
        ...
        priority: Приоритетное сообщение (пока не используется)
    """
    if not message_sender_active:
        logger.warning(f"Message sender worker неактивен, сообщение для {chat_id} отклонено")
        return False
    
    # Проверяем размер очереди
    if message_queue.qsize() > 1000:  # Максимум 1000 сообщений в очереди
        logger.warning(f"Очередь переполнена ({message_queue.qsize()}), сообщение для {chat_id} отклонено")
        return False
    ...
    try:
        message_queue.put(message_data, timeout=5)  # Максимум 5 секунд ожидания
        logger.debug(f"Сообщение добавлено в очередь для пользователя {chat_id}")
        return True
    except queue.Full:
        logger.error(f"Не удалось добавить сообщение в очередь для пользователя {chat_id}: очередь переполнена")
        return False

def test_newsletter_functionality():
    """
    Функция для тестирования системы рассылки
    
    ИСПОЛЬЗОВАНИЕ:
    - Отправляет тестовое сообщение через систему очереди
    - Проверяет статистику отправки
    - Помогает диагностировать проблемы
    """
    logger.info("🧪 Тестирование системы рассылки...")
    
    # Получаем статистику перед тестом
    stats_before = get_message_queue_status()
    logger.info(f"📊 Статистика перед тестом: {stats_before}")
    
    # Отправляем тестовое сообщение самому себе (первому администратору)
    try:
        ...
        if test_user:
            test_chat_id = test_user[0]
            test_message = f"🧪 Тестовое сообщение рассылки\n⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            
            safe_send_message(test_chat_id, test_message)
            logger.info(f"✅ Тестовое сообщение добавлено в очередь для пользователя {test_chat_id}")
            
            # Ждем немного для обработки
            time.sleep(2)
            
            # Получаем статистику после теста
            stats_after = get_message_queue_status()
            logger.info(f"📊 Статистика после теста: {stats_after}")
            
            return True
        else:
            logger.error("❌ Нет пользователей для тестирования")
            return False
            
    except Exception as e:
        logger.error(f"❌ Ошибка при тестировании рассылки: {e}")
        return False
...
def send_message_to_all_users(message_text):
    ...
    try:
        ...
        for user in users:
            try:
                if safe_send_message(user[0], message_text):
                    success_count += 1
            except Exception as e:
                logger.error(f"Ошибка при добавлении сообщения в очередь для пользователя {user[0]}: {e}")
        
        logger.info(f"Добавлено в очередь сообщений: {success_count} пользователей")
        return {
            'success': True,
            'message': f'Сообщение добавлено в очередь для {success_count} пользователей'
        }
    except Exception as e:
...
def get_newsletter_system_status():
    """
    Получить полный статус системы рассылки для админки
    
    ИСПОЛЬЗОВАНИЕ В АДМИНКЕ:
    - Проверить состояние message_sender_worker
    - Получить статистику отправки сообщений
    - Диагностировать проблемы с рассылкой
    """
    try:
        # Основная статистика
        stats = get_message_queue_status()
        ...
        conn = sqlite3.connect(DATABASE_URL)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM Users WHERE reminder = 1")
        ...
        # Вычисляем следующую пятницу 19:00
        days_until_friday = (4 - now.weekday()) % 7
        if days_until_friday == 0 and now.hour >= 19:  # Если сегодня пятница и уже после 19:00
            days_until_friday = 7
        
        next_newsletter = now + timedelta(days=days_until_friday)
        next_newsletter = next_newsletter.replace(hour=19, minute=0, second=0, microsecond=0)
        
        return {
            'worker_status': {
                'active': stats['worker_active'],
            ...
            'statistics': {
                ...
                'blocked_users_count': stats['blocked_users_count'],
                'api_errors_count': stats['api_errors_count'],
            ...
            'schedule': {
                ...
                'next_newsletter': next_newsletter.strftime('%Y-%m-%d %H:%M %Z')
            },
            'users': {
                'active_users': active_users,
                'total_users': total_users,
                'subscription_rate': active_users / total_users if total_users > 0 else 0
            }
        }
    except Exception as e:
        logger.error(f"Ошибка при получении статуса системы рассылки: {e}")
        return {
            'error': str(e)
        }

def restart_newsletter_system():
    """
    Перезапуск системы рассылки (для критических ситуаций)
    
    ИСПОЛЬЗОВАНИЕ:
    - Только в случае критических ошибок
    ...
    try:
        ...
        logger.warning("🔄 ПЕРЕЗАПУСК СИСТЕМЫ РАССЫЛКИ")
        ...
        message_stats.reset_stats()
        
        # Запускаем worker заново
        message_sender_thread = threading.Thread(target=message_sender_worker, daemon=True)
        message_sender_thread.start()
        
        logger.info("✅ Система рассылки перезапущена")
        return {
            'success': True,
            'message': 'Система рассылки перезапущена'
        }
    except Exception as e:
        logger.error(f"Ошибка при перезапуске системы рассылки: {e}")
        return {
            'success': False,
            'error': str(e)
        }
...
def handle_start(call):
    ...
    roles = [
        types.InlineKeyboardButton(text="🚀 Мини-приложение", web_app=types.WebAppInfo(url="https://gigamentor.ru"))
    ]
    markup.add(*roles)
                ...
                try:
                    async with websockets.connect(WEBSOCKET_URL) as websocket:
                        ...
                        try:
                            ...
                            markup.add(*question)
                            # Отправляем сообщение-напоминание через thread-safe очередь
                            safe_send_message(chat_id, full_answer, markup)
                            logger.info(f"Напоминание добавлено в очередь для пользователя {chat_id}")
                except Exception as e:
...
async def check_for_daily_msg():
    ...
    while True:
        try:
            ...
            # Проверяем соответствие настроенному расписанию (улучшенная логика)
            # Допускаем отправку в течение 5 минут после назначенного времени
            schedule_time = datetime.strptime(schedule['time'], '%H:%M').time()
            current_time_obj = now.time()
            
            # Проверяем, находимся ли мы в окне отправки (назначенное время ± 5 минут)
            target_datetime = now.replace(hour=schedule_time.hour, minute=schedule_time.minute, second=0, microsecond=0)
            time_diff = (now - target_datetime).total_seconds()
            
            # Окно отправки: от назначенного времени до +5 минут
            is_time_window = (current_day == schedule['day'] and 0 <= time_diff <= 300)
                    ...
                    try:
                        ...
                        try:
                            async with websockets.connect(WEBSOCKET_URL) as websocket:
                                ...
                                # Отправляем сообщение через thread-safe очередь
...
if __name__ == "__main__":
    ...
    # Запускаем worker для отправки сообщений (решение проблемы 409)
    logger.info("📨 Запускаем message sender worker...")
    message_sender_thread = threading.Thread(target=message_sender_worker, daemon=True)
    message_sender_thread.start()
    logger.info("✅ Message sender worker запущен успешно")
