...
// Определение question_id по тексту вопроса (аналогично телеграм боту)
async function detectQuestionId(questionText) {
    ...
    const questionMapping = {
        ...
        'что я могу ожидать от своего пo пm': 1,
        'что я могу ожидать от po pm': 1,
        'что ожидать от po pm': 1,
        'что я могу ожидать от своего лида': 2,
        'что я могу ожидать от лида': 2,
        'что ожидать от лида': 2,
        ...
        'что ожидать от специалиста': 4,
        'что я могу ожидать от своего po pm ': 5, // с пробелом как в боте
        'что я могу ожидать от специалиста ': 18, // с пробелом как в боте
        'что ожидается от меня': 20,
        'что от меня ожидается': 20,
        'рекомендации для стажеров': 21,
        'советы для стажеров': 21,
        'лучшие практики для стажеров': 22,
        'лучшие практики': 22,
        'что такое sdlc': 23,
        'советы по тайм менеджменту': 24,
        'тайм менеджмент': 24,
        'что еще ты умеешь': 777,
        'что ты умеешь': 777,
        'что ты можешь': 777
    };
...
async function sendQuestion() {
    ...
    try {
        // Пытаемся определить question_id по тексту вопроса
        const questionId = await detectQuestionId(question);
        ...
        // Выбираем правильный endpoint в зависимости от наличия question_id
        const askUrl = questionId 
            ? `${CONFIG.API_BASE_URL}/ask_library`  // Для известных вопросов с кешированием
            : `${CONFIG.API_BASE_URL}/ask`;         // Для свободного ввода
        
        console.log('🌐 URL для отправки:', askUrl, questionId ? `(library, question_id=${questionId})` : '(free input)');
